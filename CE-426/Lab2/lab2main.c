...
uint32_t findHighBitC(uint32_t* array, uint32_t nelements){
	// Loop backwards through all numbers in the array
	for(uint32_t i = nelements - 1; i >= 0; i--) {
		uint32_t currentNumber = array[i];
		
		// Loop backwards through all the bits in the current number
		for (int j = 31; j >= 0; j--) {
			uint32_t mask = 1u << j; // Create a mask with only the bit at position j set
			uint32_t bit = (currentNumber & mask) ? 1 : 0; // Use bitwise AND to check if the bit is set
			if(bit) // Return the current position if we find a 1
				return j + (32 * i);
    }
	}
	
	return 999; // There is an error if this is returned
}
