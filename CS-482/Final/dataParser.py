...
parser.add_argument("-l", "--logdir", help="Path of the directory to process",
                    type=str, default="logs")
parser.add_argument("-o", "--outfile", help="Name of the output file",
                    type=str, default="out.txt")
parser.add_argument("-t", "--test", help="Prints to console when true",
                    action="store_true")
...
player1_most_tspikes = 0
player2_most_spikes = 0
player2_most_tspikes = 0
...
# writes headers
def print_headers():
    for index, item in enumerate(headers.items()):
        key, value = item
        outputfile.write(f'{key}')
        # Stop the program from writing an extra comma at the end of the line
        if index < len(headers.items()) - 1:
            outputfile.write(',')
...
def reset_vars():
    ...
    global player1_most_spikes, player1_most_tspikes, player2_most_spikes, player2_most_tspikes
...
def increment_hazard_counter(hazard, player):
    ...
    if player == player1:
        if hazard == toxicspikes:
            if toxic1_counter < 2:
                toxic1_counter += 1
            if toxic1_counter > player1_most_tspikes:
                player1_most_tspikes = toxic1_counter
        elif hazard == spikes:
            if spike1_counter < 3:
                spike1_counter += 1
        ...
        if hazard == toxicspikes:
            if toxic2_counter < 2:
                toxic2_counter += 1
            if toxic2_counter > player2_most_tspikes:
                player2_most_tspikes = toxic1_counter
        elif hazard == spikes:
            if spike2_counter < 3:
                spike2_counter += 1
...
# Reset the spike counters on spike removal
# Only do this for spikes so we have an accurate count of how many layers were up at once
    ...
    for label in classlabels.keys():
        for condition in classlabels[label]:
            ...
            if condition in p1_moves:
                if condition == spikes:
                    data.append(player1_most_spikes)
                elif condition == toxicspikes:
                    data.append(player1_most_tspikes)
                else:
                    data.append('1')
            ...
            if condition in p2_moves:
                if condition == spikes:
                    data.append(player2_most_spikes)
                elif condition == toxicspikes:
                    data.append(player2_most_tspikes)
                else:
                    data.append('1')
...
print(f"{len(logs)} logs were found in dir: {logdir}.")
    ...
    with open(file_path, 'r', encoding = 'utf8') as file:         
        ...
        for line in file:
            ...
            # hazards and screens show up in the same line
                ...
                if hazard == toxicspikes or hazard == spikes:
                    if p1_name in line:                     # if p1 is in |-sidestart| line, then p2 played the hazard
                        increment_hazard_counter(hazard, player2)     # update the counter for player2
                    elif p2_name in line:
                        increment_hazard_counter(hazard, player1)     # update the counter for player1
            ...
            if is_game_over:
                ...
                if not test: 
                    # Print the data to the outputfile
                    print_data(output)
                else: 
